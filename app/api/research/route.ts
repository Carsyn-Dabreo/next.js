...
function relevanceScore(query: string, source: Source) {
  ...
  let score = 0;
  for (const term of terms) { if (title.includes(term)) score += 6; else if (text.includes(term)) score += 2; }
  if (source.type === "Knowledge Base") score += 3;
...
function rankSources(query: string, candidates: Source[]) {
  ...
  return candidates.map((source) => ({ ...source, score: relevanceScore(query, source) }))
    .filter((source) => { const key = `${normalize(source.title)}|${source.url}|${source.snippet.slice(0, 80)}`; if (seen.has(key) || (source.score ?? 0) < 2) return false; seen.add(key); return true; })
...
function retrieveKBChunks(query: string, chunks: KBChunk[]) {
  ...
  return chunks.map((chunk) => {
    const text = normalize(chunk.text); const hits = terms.reduce((count, term) => count + (text.includes(term) ? 1 : 0), 0);
...
async function retrieveSources(query: string): Promise<Source[]> {
  ...
  candidates.push(...retrieveKBChunks(query, kb.chunks));
  ...
  const results = await Promise.allSettled(jobs); for (const result of results) if (result.status === "fulfilled") candidates.push(...result.value); return rankSources(query, candidates);
}

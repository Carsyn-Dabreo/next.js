...
import { ArrowRight, BookOpen, CheckCircle2, Clock3, Download, ExternalLink, FileSearch, FolderOpen, LayoutDashboard, LogOut, MoreVertical, Plus, Search, Settings, ShieldCheck, Sparkles, Target, Trash2, Upload, Users, X } from "lucide-react";
...
const navItems = [{ id: "dashboard", label: "Dashboard", icon: LayoutDashboard }, { id: "research", label: "Research", icon: Sparkles }, { id: "projects", label: "Projects", icon: FolderOpen }, { id: "knowledge", label: "Knowledge Base", icon: BookOpen }, { id: "history", label: "History", icon: Clock3 }, { id: "collaborators", label: "Collaborators", icon: Users }, { id: "settings", label: "Settings", icon: Settings }];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    try {
      setProjects(JSON.parse(localStorage.getItem(`rcc_projects_${emailKey}`) || "[\"Enterprise AI Research\",\"RAG Evaluation\"]"));
      setInvited(JSON.parse(localStorage.getItem(`rcc_invites_${emailKey}`) || "[]"));
    } catch {}
  ...
  const invite = () => { const value = inviteEmail.trim().toLowerCase(); if (!value || !value.includes("@")) return; const next = [...new Set([...invited, value])]; setInvited(next); localStorage.setItem(`rcc_invites_${emailKey}`, JSON.stringify(next)); setInviteEmail(""); };
  ...
  const renderCollaborators = () => <div className="card section-card large-section"><div className="cardhead"><div><h2>Collaborators</h2><p>Prepare a project invite list. Email delivery can be connected later to an email provider.</p></div></div><div className="inline-form"><input value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} placeholder="colleague@example.com" type="email" onKeyDown={(e) => e.key === "Enter" && invite()}/><button className="primary" onClick={invite}><Users size={15}/>Add Invite</button></div><div className="member-list"><div className="member"><div className="member-avatar">{initials}</div><div><strong>{displayName}</strong><span>Owner · Google account</span></div><span className="status">Active</span></div>{invited.map((email) => <div className="member" key={email}><div className="member-avatar">{email[0].toUpperCase()}</div><div><strong>{email}</strong><span>Pending collaborator invite</span></div><span className="status">Ready</span></div>)}</div></div>;
